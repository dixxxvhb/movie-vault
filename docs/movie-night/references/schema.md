...
| Object | Use |
|---|---|
| `film_status` (view) | One row per title with computed `state`: ledger / certified / archive / hazy / seen / abandoned / queued / watching / pitched / dismissed / veto / fresh. Precedence is baked in: ledger > certified > archive > hazy (bucket set, no score) > seen (note, no bucket) > abandoned > veto > queued > watching > pitched > dismissed > fresh. Plus `live_score, live_watched_at, is_rewatch_seen, memory_score, seen_bucket, certified_score, open_rec_id, last_pitched_on, pitch_count, runtime_minutes, providers, provenance`. |
| `film_check(q text)` | Trigram match on `film_status` (`similarity > 0.35`, ordered by similarity then year desc). Returns the film_status columns plus `similarity` and a `verdict` text, e.g. `ON THE WALL 9.8 (2026-08-04)`, `ARCHIVE memory 9.6, rewatch only, disclose`, `HAZY, rewatch only, disclose`, `SEEN, unscored, confirm before pitching`, `OPEN REC since 2026-08-15`, `QUEUED (<provenance>)`, `VETO: only Dixon raises it`, `FRESH`. A `film_vetoes` row overrides every other verdict, whatever the state says. Multiple years of the same name come back as separate rows; that is the version check. |
